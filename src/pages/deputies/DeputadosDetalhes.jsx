...
const DeputadosDetalhes = (props) => {
	...
	function badgeColor() {
		if (deputados.ultimoStatus.situacao === 'Exercício' || deputados.ultimoStatus.situacao === 'Suplência') {
			...
			deputados.ultimoStatus.situacao === 'Afastado' ||
			deputados.ultimoStatus.situacao === 'Licença' ||
			deputados.ultimoStatus.situacao === 'Suspenso'
		) {
			return 'warning';
		} else if (
			deputados.ultimoStatus.situacao === 'Fim de Mandato' ||
			deputados.ultimoStatus.situacao === 'Vacância'
		) {
			return 'danger';
		} else {
			return 'secondary';
				...
				<>
					<Container>
						...
						<Row style={{ paddingTop: '20px' }}>
							<Col md={1}></Col>
							<Col md={3} style={{ textAlign: 'right', paddingRight: '0', position: 'relative' }}>
								<img
									src={deputados.ultimoStatus.urlFoto}
									alt={deputados.ultimoStatus.nomeEleitoral}
									style={{ width: '250px' }}
								/>
								<center>
									<Badge bg="secondary" style={{ top: '50px', borderRadius: '15px' }}>
										<Link to={`/partidos/${getPartido()}`} className="noUnderline">
											{deputados.ultimoStatus.siglaPartido}-{deputados.ultimoStatus.siglaUf}
										</Link>
									</Badge>
								</center>
							</Col>
							...
							<Col md={7}>
								...
								<h2>
									...
									{isFavorite() && (
										<AiFillStar style={{ fontSize: '1.5rem' }} onClick={handleRemoveFavorite} />
									)}
									{!isFavorite() && (
										<AiOutlineStar style={{ fontSize: '1.5rem' }} onClick={handleFavorite} />
									)}
								</h2>
								<Badge pill bg={badgeColor()} style={{ fontSize: '14px', verticalAlign: '35%' }}>
									{' '}
									{deputados.ultimoStatus.situacao}
								</Badge>
								<br />
